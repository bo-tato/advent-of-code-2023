  ...
  (map-combinations
   (lambda (lines)
     (ignore-errors (bind ((((x1 y1 _ vx1 vy1 _) (x2 y2 _ vx2 vy2 _)) lines)
                           ((intersection-x intersection-y) (line-intersection [x1 y1 vx1 vy1] [x2 y2 vx2 vy2])))
...
(bind ((vx (find-velocity #'first #'fourth))
       ...
       ((x1 y1 z1 vx1 vy1 vz1) (first *in*))
       ((x2 y2 z2 vx2 vy2 vz2) (second *in*))
       ((x y) (line-intersection [x1 y1 (- vx1 vx) (- vy1 vy)]
                                 [x2 y2 (- vx2 vx) (- vy2 vy)]))
       (time (floor (- x x1) (- vx1 vx)))
       (z (+ z1 (* time (- vz1 vz)))))
  (+ x y z))
